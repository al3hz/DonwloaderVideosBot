...
- **Eager bootstrap:** Under gunicorn (module imported, `__name__ != "__main__"`), a fire-and-forget daemon thread calls `ensure_bot()` at import time so the webhook (with secret) is configured right after boot, without waiting for the first update.
