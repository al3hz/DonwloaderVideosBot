...
3. **Spotify music (`spotdl`):** Links de open.spotify.com interceptados ANTES de la cola: `_start_spot_listing()` corre `spotdl save` (subprocess, credenciales default incluidas) → lista numerada HTML + botones `spt:<job>:<idx|all>` (job cacheado en `_spotdl_jobs`, trim a 20, solo el owner puede tocar). Al tocar, se encola DownloadTask con `meta={"kind":"spotdl", ...}` y `_execute_download` deriva a `_execute_spotdl_task`: match propio en YT Music (`ytsearch5` + score por duración ±20s) → bestaudio → ffmpeg MP3 192k → sendAudio con cover como thumbnail. **CRITICO: YouTube requiere deno (JS runtime) o la descarga da 403** — el Build Command deja ./bin/deno y main.py lo antepone al PATH; `/health` reporta `"deno": true/false`. Tope por lista: SPOTDL_MAX_TRACKS (default 15). NO usar el proveedor de descarga interno de spotdl (fragil ante anti-bot).
