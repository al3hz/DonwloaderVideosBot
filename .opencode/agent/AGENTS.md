...
3. **Spotify music (`spotdl`):** Links de open.spotify.com interceptados ANTES de la cola: `_start_spot_listing()` corre `spotdl save` (subprocess, credenciales default incluidas) → lista numerada HTML + botones `spt:<job>:<idx|all>` (job cacheado en `_spotdl_jobs`, trim a 20, solo el owner puede tocar). Al tocar, se encola DownloadTask con `meta={"kind":"spotdl", ...}` y `_execute_download` deriva a `_execute_spotdl_task`: match propio en YT Music (`ytsearch5` + score por duración ±20s) → bestaudio → ffmpeg MP3 192k → sendAudio con cover como thumbnail. **CRITICO: YouTube requiere deno (JS runtime) o la descarga da 403, y desde IPs de datacenter (Render) TAMBIEN exige cookies ("Sign in to confirm you're not a bot")** — `_apply_yt_guard()` inyecta en TODA extraccion de youtube: `cookiefile` (decodificado de env var `YOUTUBE_COOKIES_B64`, base64 del cookies.txt), `js_runtimes deno` (Build deja ./bin/deno, main.py lo antepone al PATH) y `proxy` si OUTBOUND_PROXY_URL. `/health` reporta `"deno": true/false`. Tope por lista: SPOTDL_MAX_TRACKS (default 15). Calidad de audio SIEMPRE VBR max (`preferredquality="0"` — YouTube entrega ~128-160k reales; fijar 320k solo infla). NO usar el proveedor de descarga interno de spotdl (fragil ante anti-bot).
4. **Queue System (FIFO per user):** Each URL received is validated and then enqueued into a per-user `asyncio.Queue`. A dedicated background worker (`_queue_worker`) processes tasks in order. Workers auto-terminate after 5 minutes of inactivity. Spotify track taps also enqueue here (via `spot_cb`).
5. **TikTok Slideshows:** Detect `/photo/` URLs. Bypass `yt-dlp` and utilize the `tikwm.com` API as a robust fallback. IMPORTANT: yt-dlp does NOT support `/photo/` posts nor extract slideshow images (only their audio) — verified against yt-dlp master `_VALID_URL`. tikwm is accessed via `curl_cffi` with browser TLS impersonation rotating profiles (`_TIKWM_PROFILES`: chrome → firefox133 → chrome124) plus browser headers, because Cloudflare blocks datacenter IPs; a 1.1s pause between attempts respects tikwm's free tier (1 req/s). If `OUTBOUND_PROXY_URL` env var is set, it is used for tikwm calls, direct media downloads AND yt-dlp (`opts["proxy"]`).
...
7. **Twitter media fallback:** When yt-dlp fails on a tweet, `_twitter_media_fallback()` classifies via fxtwitter/vxtwitter into `(image_urls, gif_urls)` — X serves GIFs as MP4 (`tweet_video/*.mp4`, `type:"gif"`). Images go to a photo media group; GIFs MUST be sent via `send_animation` (sending them as photos shows a static frame) using `_download_images(..., default_ext=".mp4")`. Never filter fallback media to `type=="image"` only.
7. **Delivery Phase with Retry:** Upload via Telegram (`HTTPXRequest` defaults: `connect_timeout=30`, `read_timeout=120`, `write_timeout=30`, `media_write_timeout=300`, `pool_timeout=30`, `connection_pool_size=256`; per-call overrides use `read_timeout=120`, `write_timeout=120`, `connect_timeout=30`). File limits: videos/GIFs `< 50 MB`, photos `< 10 MB` (Telegram rejects bigger photos). **50MB rescue (proven E2E):** when the final generic download exceeds `MAX_FILE_BYTES`: (1) RENAME the oversized file to `rescue_input_<uuid>.mp4` — name WITHOUT the video id, so `_find_downloaded_file()`'s cwd sweep can't pick it up again, and kept as guaranteed ffmpeg input; (2) best-effort ladder `_DOWNLOAD_RESCUE_FORMATS` via `download(fmt, cached_info=first_info)` (`check_formats=False`; NOTE: re-extraction AND cached-info selection both fail on Reddit because repeat video-info fetches get throttled and post-download formats lose their `url`s — the ladder only rescues platforms whose cached formats keep urls); (3) LAST RESORT that actually works: `_ffmpeg_downscale_720()` re-encodes the renamed file to ≤720p CRF28/**ultrafast** with a **420s** timeout — Render's shared CPU is ~4-6x slower than a dev machine and veryfast@240s TIMED OUT in production; rescues also `opts.pop("format_sort")` because format sorting against cached info raises `'LazyList' object has no attribute 'sort'` (E2E: 55.4MB→15.8MB; user sees "⏳ Convirtiendo..." while encoding). ffmpeg availability is reported by `/health` as `"ffmpeg": true/false`. All `send_video`/`send_animation`/`send_photo` calls are wrapped in `_send_file_with_retry()` which re-opens the file and retries up to 3 times on `TimedOut`/`NetworkError` with exponential backoff. If the file has no audio stream, it is sent as `send_animation` (GIF) instead of `send_video`. All uploads use `application.bot.send_*()` (not `update.message.reply_*()`) because the download runs in the queue worker context. **Chat Action:** Before each upload, `_send_chat_action()` is called (throttled to max 1 per ~4.5s per chat) with the appropriate action (`UPLOAD_VIDEO` for videos/GIFs, `UPLOAD_PHOTO` for photos/albums) so Telegram shows a "subiendo..." status to the user.
...
- **Webhook tuning:** `allowed_updates=["message", "callback_query"]` (callback_query es necesario para los botones de `/config`), `drop_pending_updates=True`, `max_connections=40`.
...
- **User settings (`/config`):** Per-user preferences live in `_user_settings` (in-memory, reset on deploy). `_build_caption(task, title)` is the ONLY place captions are constructed: it respects `show_credit` ("Descargado por @bot") and `show_title` (video title/description). Never hardcode caption strings — always use `_build_caption`. The `/config` inline keyboard uses callback_data prefix `cfg:` handled by `config_cb` via `CallbackQueryHandler`.
...
| `SPOTDL_MAX_TRACKS`        | ❌       | `15`                    | Tope de pistas por lista de Spotify              |
| `UPSTASH_REDIS_REST_URL`   | ❌       | —                       | URL REST de Upstash: persiste settings por usuario + stats |
| `UPSTASH_REDIS_REST_TOKEN` | ❌       | —                       | Token REST de Upstash                            |
| `OUTBOUND_PROXY_URL`       | ❌       | —                       | Proxy HTTP/SOCKS5 para TikTok/tikwm/yt-dlp cuando la IP del host está bloqueada por Cloudflare |
